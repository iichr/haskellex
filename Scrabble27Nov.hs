...
invertBoardBram = Bram.invertBoard
invertBoard b = reverse [reverse x | x <- b] 
...
--pickUniTest :: LRand Rack
pickUniTest xs = [pickUniformly x | x <- xs]
...
replenishRackOrig rack = let Bram.LRand f = Bram.replenishRack rack in LRand f
replenishRack rack = do
    x <- sequence $ replicate (rackSize-length(rack)) newLetter
    return (rack ++ x)
...
-- HELPER FUNCTIONS FOR ALLWORDS2
...
-- below is contingent on tupConverterList, avoid using. 18/11
...
letterStreamBram = let Bram.LRand f = Bram.letterStream in LRand f
